...
with col1:
    ...
    if st.session_state.dernier_personnage is not None and st.session_state.dernier_personnage != nom:
        ...
        st.session_state.historique_chat = []
        st.toast(f"✨ Nouvelle conversation avec {nom} ! L'ancienne a été effacée ✨", icon="🔄")
...
with col2:
    ...
    with col_clear:
        ...
        if st.button("🗑️ Effacer", use_container_width=True, help="Effacer toute la conversation"):
            st.session_state.historique_chat = []
            st.toast("💬 Conversation effacée ! Tu peux recommencer à discuter ✨", icon="🗑️")
            st.rerun()
